...
**Notebooks:**
- [`notebooks/01_baseline_model.ipynb`](notebooks/01_baseline_model.ipynb) — Linear Regression and Random Forest on the baseline feature set, plus initial time/route-popularity feature engineering
- [`notebooks/02_final_enriched_model.ipynb`](notebooks/02_final_enriched_model.ipynb) — the full pipeline, adding local events-data enrichment and feature importance analysis
...
```
├── notebooks/
│   ├── 01_baseline_model.ipynb          # LR + RF on baseline features, initial feature engineering
│   └── 02_final_enriched_model.ipynb    # + events data enrichment, feature importance analysis
├── docs/
...
jupyter notebook notebooks/   # open 01_baseline_model.ipynb, then 02_final_enriched_model.ipynb
```

**Note on data:** the raw trip-level dataset (`final_merged_data.csv`) is not
included in this repository due to its size and the academic data-sharing
arrangement it was provided under. Both notebooks are included with their
original outputs, so the full analysis and results are visible without
needing the raw file.
...
Venkatesh Komakula — [LinkedIn](https://www.linkedin.com/in/venkatesh-ai-engineer/)
